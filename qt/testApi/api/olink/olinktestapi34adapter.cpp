...
#include "olinktestapi34adapter.h"
...
OLinkTestApi34Adapter::OLinkTestApi34Adapter(RemoteRegistry& registry, AbstractTestApi34* impl, QObject *parent)
    : QObject(parent)
    , m_impl(impl)
    , m_registry(registry)
{
    connect(m_impl, &AbstractTestApi34::propIntChanged, this,
        [=](int propInt) {
    ...
    connect(m_impl, &AbstractTestApi34::propFloatChanged, this,
        [=](qreal propFloat) {
    ...
    connect(m_impl, &AbstractTestApi34::propStringChanged, this,
        ...
        connect(m_impl, &AbstractTestApi34::sigInt, this,
            [=](int paramInt) {
        ...
        connect(m_impl, &AbstractTestApi34::sigFloat, this,
            [=](qreal paramFloat) {
        ...
        connect(m_impl, &AbstractTestApi34::sigString, this,
...
json OLinkTestApi34Adapter::captureState()
{
    return json::object({
...
void OLinkTestApi34Adapter::applyState(const json& state)
{
...
std::string OLinkTestApi34Adapter::olinkObjectName() {
    return "api.TestApi34";
}

json OLinkTestApi34Adapter::olinkInvoke(const std::string& methodId, const nlohmann::json& args){
...
void OLinkTestApi34Adapter::olinkSetProperty(const std::string& propertyId, const nlohmann::json& value){
    AG_LOG_DEBUG(Q_FUNC_INFO);
...
void OLinkTestApi34Adapter::olinkLinked(const std::string& objectId, IRemoteNode *node) {
    AG_LOG_INFO(Q_FUNC_INFO);
    AG_LOG_INFO(objectId);
}

void OLinkTestApi34Adapter::olinkUnlinked(const std::string& objectId)
{
    AG_LOG_INFO(Q_FUNC_INFO);
    AG_LOG_INFO(objectId);
}

json OLinkTestApi34Adapter::olinkCollectProperties()
{
    return captureState();
}
