...
#include "olinktestapi77adapter.h"
...
OLinkTestApi77Adapter::OLinkTestApi77Adapter(RemoteRegistry& registry, AbstractTestApi77* impl, QObject *parent)
    : QObject(parent)
    , m_impl(impl)
    , m_registry(registry)
{
    connect(m_impl, &AbstractTestApi77::propIntChanged, this,
        [=](int propInt) {
    ...
    connect(m_impl, &AbstractTestApi77::propFloatChanged, this,
        [=](qreal propFloat) {
    ...
    connect(m_impl, &AbstractTestApi77::propStringChanged, this,
        ...
        connect(m_impl, &AbstractTestApi77::sigInt, this,
            [=](int paramInt) {
        ...
        connect(m_impl, &AbstractTestApi77::sigFloat, this,
            [=](qreal paramFloat) {
        ...
        connect(m_impl, &AbstractTestApi77::sigString, this,
...
json OLinkTestApi77Adapter::captureState()
{
    return json::object({
...
void OLinkTestApi77Adapter::applyState(const json& state)
{
...
std::string OLinkTestApi77Adapter::olinkObjectName() {
    return "api.TestApi77";
}

json OLinkTestApi77Adapter::olinkInvoke(const std::string& methodId, const nlohmann::json& args){
...
void OLinkTestApi77Adapter::olinkSetProperty(const std::string& propertyId, const nlohmann::json& value){
    AG_LOG_DEBUG(Q_FUNC_INFO);
...
void OLinkTestApi77Adapter::olinkLinked(const std::string& objectId, IRemoteNode *node) {
    AG_LOG_INFO(Q_FUNC_INFO);
    AG_LOG_INFO(objectId);
}

void OLinkTestApi77Adapter::olinkUnlinked(const std::string& objectId)
{
    AG_LOG_INFO(Q_FUNC_INFO);
    AG_LOG_INFO(objectId);
}

json OLinkTestApi77Adapter::olinkCollectProperties()
{
    return captureState();
}
